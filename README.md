...
[**Download `selvard-v0.6.0-phase10.apk` (v0.6.0, Phase 10)**](https://github.com/Khubaib7-del/MARK-42/releases/download/v0.6.0-phase10/selvard-v0.6.0-phase10.apk) — sideload on any Android 8.0+ device. Bottom-tab navigation (Home, Security, Network, Apps, Identity, Timeline, Settings): Home shows posture in words with honesty limits, Security pairs link checks with the posture reading, and Settings adds recorded-data deletion with a receipt. Includes Link Guardian (share or paste a link), App Guardian (on-demand app inventory scan), the opt-in Network Guardian DNS filter, Privacy Monitor (install/update/remove facts, permission snapshots, boot records, device-integrity facts with honest NOT MONITORED labels), Identity Exposure (consented HIBP breach checks — needs your own HIBP API key), and the Incident Timeline (temporal grouping of recorded signals — never causal). Phase 10 fixes 13 in-tree hardening findings (H1–H13: VPN fail-closed holes, share-input bounds, HIBP case-miss + body bounds, vault validation, tap-jacking guard, FLAG_SECURE, cleartext ban). **Includes the v0.1.1 scan-crash fix.** Debug-signed for testing; the release notes list every honest limit of this build.
...
**Hardening** (Phase 10): full report in `docs/security/HARDENING_REPORT.md` — 13 findings fixed, 138/138 JVM tests green. No external pen test; no on-device run (no emulator in the build sandbox); IPv6 DNS bypass, single-threaded pump, battery benchmark, and corpus-scale accuracy carry to Phase 11; biometric gate and a dependency-vulnerability scan are open follow-ups.
