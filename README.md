...
[**Download `selvard-v0.5.0-phase9.apk` (v0.5.0, Phase 9)**](https://github.com/Khubaib7-del/MARK-42/releases/download/v0.5.0-phase9/selvard-v0.5.0-phase9.apk) — sideload on any Android 8.0+ device. Now with bottom-tab navigation (Home, Security, Network, Apps, Identity, Timeline, Settings) instead of the pager: Home shows posture in words with honesty limits, Security pairs link checks with the posture reading, and Settings adds recorded-data deletion with a receipt. Includes Link Guardian (share or paste a link), App Guardian (on-demand app inventory scan), the opt-in Network Guardian DNS filter, Privacy Monitor (install/update/remove facts, permission snapshots, boot records, device-integrity facts with honest NOT MONITORED labels), Identity Exposure (consented HIBP breach checks — needs your own HIBP API key), and the Incident Timeline (temporal grouping of recorded signals — never causal). **Includes the v0.1.1 scan-crash fix.** Debug-signed for testing; the release notes list every honest limit of this build.
