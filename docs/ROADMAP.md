...
| **1 — Project foundation** | Gradle multi-module scaffold (/core KMP + /android), CI skeleton (format, lint, detekt, gitleaks, unit tests), minimal Compose shell app with disclosure-first onboarding | CI green on empty engines; KMP domain module builds for jvm+android; no production permissions beyond INTERNET |
| **2 — Security core** | Event model + event bus + encrypted Room event store + risk engine skeleton + policy engine + retention enforcement | Unit tests for event round-trip, encryption, risk determinism, retention deletion; keystore-backed key ring on device |
